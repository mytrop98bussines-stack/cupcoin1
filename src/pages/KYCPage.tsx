...
import {
  Shield, Upload, Camera, User, CheckCircle2,
  Clock, AlertTriangle, FileText, MapPin,
  RefreshCw, Lock, ChevronRight, X,
} from "lucide-react";
...
useEffect(() => {
  ...
  const checkStatus = async () => {
    ...
    try {
      ...
      const res   = await fetch(
        "https://cubax-backend.onrender.com/api/kyc/status",
        { headers: { Authorization: `Bearer ${token}` } }
      );
      const data = await res.json();

      if (data.success && !stopped) {
        setUser({ ...user, kycStatus: data.kycStatus });
      }
    } catch (err) {
      console.error("❌ Error verificando estado KYC:", err);
    }
  };
  ...
  const handleFileChange = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>, type: "id" | "selfie") => {
      ...
      formData.append("folder", `cubax/kyc/${user?.uid}/${type}`);
      ...
      try {
        ...
        if (type === "id") setIdFront(data.secure_url);
        else setSelfie(data.secure_url);
      } catch (error: any) {
        setUploadError(
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      const token = localStorage.getItem("cubax_token");
      const res   = await fetch(`${BACKEND_URL}/kyc/submit`, {
        method:  "POST",
        headers: {
    ...
    return (
      <div className="max-w-lg mx-auto px-4 py-16 text-center animate-fade-in">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 max-w-xs mx-auto mb-4">
          Tu cuenta está completamente verificada. Puedes operar sin restricciones en CubaX.
        </p>
        <Badge variant="success" size="md">✓ KYC Aprobado</Badge>
        <div className="mt-6 grid grid-cols-2 gap-3 max-w-xs mx-auto">
          <div className="p-3 rounded-xl bg-emerald-500/5 border border-emerald-500/20 text-center">
            <p className="text-xs font-bold text-emerald-600 dark:text-emerald-400">Sin límites</p>
            <p className="text-[10px] text-gray-400">en P2P</p>
          </div>
          <div className="p-3 rounded-xl bg-emerald-500/5 border border-emerald-500/20 text-center">
            <p className="text-xs font-bold text-emerald-600 dark:text-emerald-400">Acceso total</p>
            <p className="text-[10px] text-gray-400">al Marketplace</p>
          </div>
        </div>
        <Button size="lg" className="mt-6 w-full max-w-xs mx-auto" onClick={() => navigate("dashboard")}>
          Ir al Dashboard
        </Button>
      </div>
    );
  ...
  if (submitted || user.kycStatus === "pending_verification") {
    return (
      <div className="max-w-lg mx-auto px-4 py-16 text-center animate-fade-in">
        ...
        <h2 className="text-xl font-bold text-gray-900 dark:text-white mb-2">En revisión</h2>
        <p className="text-sm text-gray-500 dark:text-gray-400 max-w-xs mx-auto mb-4">
          Tus documentos están siendo revisados. Te notificaremos cuando se complete.
        </p>
          ...
          {[
            ...
            <div key={item.label} className="flex items-center gap-2">
              <div className={`h-5 w-5 rounded-full flex items-center justify-center flex-shrink-0 ${
                item.done ? "bg-emerald-500" : "bg-gray-200 dark:bg-white/10"
              }`}>
                {item.done
  ...
  if (user.kycStatus === "rejected") {
    return (
      <div className="max-w-lg mx-auto px-4 py-16 text-center animate-fade-in">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 max-w-xs mx-auto mb-4">
          Tu solicitud fue rechazada. Vuelve a intentarlo con documentos más claros.
        </p>
        <Button
          size="lg"
          className="w-full max-w-xs mx-auto"
          onClick={() => { setStep(0); setIdFront(null); setSelfie(null); setErrors({}); }}
        >
          Intentar de nuevo
        </Button>
      </div>
  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-4 pb-24 space-y-4 animate-fade-in">
      ...
      {/* Inputs ocultos */}
      <input type="file" accept="image/*" capture="environment" ref={idInputRef}
        onChange={(e) => handleFileChange(e, "id")} className="hidden" />
      <input type="file" accept="image/*" capture="user" ref={selfieInputRef}
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-lg font-bold text-gray-900 dark:text-white">Verificación KYC</h1>
          <p className="text-xs text-gray-500 dark:text-gray-400">Paso {step + 1} de {steps.length}</p>
        </div>
      </div>
      ...
      <div className="space-y-2">
        <div className="flex gap-1.5">
          {steps.map((s, i) => (
            <div key={s.label} className={`flex-1 h-1.5 rounded-full transition-all duration-500 ${
              i <= step ? "bg-brand-500" : "bg-gray-200 dark:bg-white/10"
            }`} />
          ))}
        </div>
        <div className="flex items-center gap-1.5">
          <span className="text-brand-500">{steps[step].icon}</span>
      ...
      <Card padding="sm" className="border-blue-500/20 bg-blue-50 dark:bg-blue-500/5">
        <div className="flex items-start gap-2">
          ...
          <p className="text-xs text-gray-600 dark:text-gray-400 leading-relaxed">
            Tus documentos se almacenan de forma cifrada. Solo nuestro equipo de verificación tiene acceso.
          </p>
        </div>
      </Card>
      ...
      {step === 0 && (
        <div className="space-y-4 animate-slide-up">
          ...
          <Input
            ...
            value={idNumber}
            onChange={(e) => { const val = e.target.value.replace(/\D/g, "").slice(0, 11); setIdNumber(val); if (errors.idNumber) setErrors((p) => ({ ...p, idNumber: "" })); }}
          ...
          />
          <Button size="lg" fullWidth onClick={() => { if (validateStep0()) setStep(1); }} icon={<ChevronRight className="h-4 w-4" />}>
            Continuar
          </Button>
        </div>
      )}
      ...
      {step === 1 && (
        <div className="space-y-4 animate-slide-up">
          <p className="text-sm text-gray-600 dark:text-gray-400 leading-relaxed">
            Sube una foto clara del <strong>frente de tu carné de identidad</strong>.
          </p>
          <button
          ...
          >
            {uploadingType === "id" ? (
              <div className="space-y-2 py-4">
                ...
                <p className="text-xs text-gray-400 animate-pulse">Subiendo documento...</p>
              </div>
            ) : idFront ? (
              <div className="space-y-3">
                <img src={idFront} alt="Documento" className="w-48 h-32 object-cover rounded-xl mx-auto shadow-md" />
                <div className="flex items-center justify-center gap-1.5">
                  <CheckCircle2 className="h-4 w-4 text-emerald-500" />
                  <p className="text-xs font-bold text-emerald-600 dark:text-emerald-400">Documento subido correctamente</p>
                </div>
                <p className="text-[10px] text-gray-400">Toca para cambiar la imagen</p>
              </div>
            ) : (
              <div className="space-y-2">
                ...
                <p className="text-sm font-semibold text-gray-700 dark:text-gray-300">Toca para abrir cámara o galería</p>
                <p className="text-[10px] text-gray-400">JPG o PNG • Máx. 5MB</p>
              </div>
            )}
          </button>
          <div className="space-y-1.5">
            {["Imagen nítida y bien iluminada", "Todos los datos legibles", "Sin reflejos ni sombras"].map((tip) => (
          ...
          <div className="flex gap-2">
            <Button size="lg" variant="outline" onClick={() => setStep(0)} disabled={uploadingType !== null} className="flex-1">Atrás</Button>
            <Button size="lg" onClick={() => setStep(2)} disabled={!idFront || uploadingType !== null} className="flex-1" icon={<ChevronRight className="h-4 w-4" />}>Continuar</Button>
          </div>
        </div>
      )}
      ...
      {step === 2 && (
        <div className="space-y-4 animate-slide-up">
          <p className="text-sm text-gray-600 dark:text-gray-400 leading-relaxed">
            Tómate una <strong>selfie sosteniendo tu CI</strong> junto a tu rostro.
          </p>
          <button
          ...
          >
            {uploadingType === "selfie" ? (
              <div className="space-y-2 py-4">
                <div className="h-7 w-7 border-2 border-brand-500 border-t-transparent rounded-full animate-spin mx-auto" />
                <p className="text-xs text-gray-400 animate-pulse">Subiendo selfie...</p>
              </div>
            ) : selfie ? (
              <div className="space-y-3">
                <img src={selfie} alt="Selfie" className="w-32 h-32 object-cover rounded-full mx-auto shadow-md border-4 border-emerald-500/20" />
                <div className="flex items-center justify-center gap-1.5">
                  <CheckCircle2 className="h-4 w-4 text-emerald-500" />
                  <p className="text-xs font-bold text-emerald-600 dark:text-emerald-400">Selfie subida correctamente</p>
                </div>
                <p className="text-[10px] text-gray-400">Toca para cambiar la selfie</p>
              </div>
            ) : (
              <div className="space-y-2">
                ...
                <p className="text-sm font-semibold text-gray-700 dark:text-gray-300">Toca para capturar selfie</p>
                <p className="text-[10px] text-gray-400">Buena iluminación • Rostro y CI visibles</p>
              </div>
            )}
          </button>
          ...
          <div className="flex gap-2">
            <Button size="lg" variant="outline" onClick={() => setStep(1)} disabled={uploadingType !== null || loading} className="flex-1">
              Atrás
            </Button>
            <Button size="lg" onClick={handleSubmit} disabled={!selfie || uploadingType !== null} loading={loading} className="flex-1" icon={<Shield className="h-4 w-4" />}>
              Enviar KYC
            </Button>
          </div>
        </div>
        ...
        <div className="flex items-start gap-2">
          ...
          <div>
            ...
            <p className="text-[11px] text-gray-500 dark:text-gray-400 leading-relaxed">
              Sin verificación tienes límites de volumen en P2P y el Marketplace.
              Verifica para operar sin restricciones.
            </p>
          </div>
        </div>
      </Card>
