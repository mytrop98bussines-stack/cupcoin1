...
const handleGoogleLogin = async () => {
  ...
  try {
    // 1. Google Popup — funciona en Cuba ✅
    const provider = new GoogleAuthProvider();
    const result   = await signInWithPopup(auth, provider);
    const idToken  = await result.user.getIdToken();

    // 2. Enviar idToken al backend
    // El backend crea/actualiza el usuario en Firestore
    ...
    if (!data.success) {
      setGlobalError(data.error || "Error con Google.");
      return;
    }
    ...
    const appUser: AppUser = {
      ...
      kycStatus:     data.userData?.kycStatus   || "unverified",
      createdAt:     data.userData?.createdAt   || Date.now(),
      totalTrades:   data.userData?.totalTrades || 0,
      rating:        data.userData?.rating      || 5.0,
